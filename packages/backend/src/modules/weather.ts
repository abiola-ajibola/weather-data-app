...
const numericFields = new Set<keyof WeatherObservationRow>([
  'latitude',
  'longitude',
  'elevationM',
  'prcp',
...
const stringFields = new Set<keyof WeatherObservationRow>([
  'stationId',
  'stationName',
  'prcpAttributes',
  'tmaxAttributes',
    ...
    if (sortBy === 'date') {
      return (
        (new Date(String(leftValue)).getTime() -
          new Date(String(rightValue)).getTime()) * direction
      )
    }
